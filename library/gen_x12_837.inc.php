...
function gen_x12_837($pid, $encounter, &$log, $encounter_claim=false) {
  ...
  for ($prockey = 0; $prockey < $proccount; ++$prockey) {
    ...
    if (!$CMS_5010) {
	  ...
	  $aprvd_amnt=0;
	  //new method Eldho
	  $cpt=$claim->cptKey($prockey);
	  foreach($aarr[$cpt]['pmnt'] as $k=>$v)
	  {
		$aprvd_amnt += $v['pay_amount']+$v['ded_amount'];
    ...
    for ($ins = 1; $ins < $claim->payerCount(); ++$ins) {
	  ...
	  $aprvd_amnt=0;
	  $paid_amnt=0;
	  foreach($aarr[$prockey]['pmnt'] as $k=>$v)
	  {
		//$aprvd_amnt += $v['pay_amount']+$v['ded_amount'];
		$paid_amnt += $v['pay_amount'];
	  }
	   if(!$medicaresec){
      ...
      foreach ($aarr as $a) {
		...
		//find out which groupcodes are there in the payment, and details are stored into an array
		//
		$cpt=$claim->cptKey($prockey);
		//print_r($aarr[$cpt]['pmnt']);
		//die;
		$a=null;
		foreach($aarr[$cpt]['pmnt'] as $k=>$v)
		{
			if($v['grp_code'])
			$a[$v['grp_code']][]=array('rsn_code'=>$v['rsn_code'],'amnt'=>($v['adj_amount']+$v['ded_amount']));
