...
class Claim {
 ...
 // echo $rs_chg.$this->encounter_id;
  $rs_pmnt=sqlStatement("select code,modifier,pay_amount,adj_amount,ded_amount,grp_code,rsn_code 
  from ar_activity where encounter=? and payer_type=1 ",array($this->encounter_id));
  //echo $rs_chg;
  //die("iam here");
  $total_allowed=0;
  $total_paid=0;
  ...
  $arr[$code_mod]['fee'] +=$row['fee'];
  }
  $pmnt=array();
  while($row=sqlFetcharray($rs_pmnt)){
  $total_allowed +=($row['pay_amount']+$row['ded_amount']);
  $total_paid +=$row['pay_amount'];
  if($row['grp_code']=='PR'){
  ...
  $arr['totalallowed']=$total_allowed;
  $arr['totalpatresp']=$total_pat_resp;
  return $arr;
  }
  else{
 ...
 /* function payerAdjustments($ins, $code='Claim') {
  ...
  //
  function claimAdjudicationDate(){
  //only primary insurance tertiary ins not handling
  $sql="select date_format(ins_adjud_date,'%Y%m%d') as dt from ar_activity where encounter=? 
   and payer_type=1 and ins_adjud_date is not null and ins_adjud_date<>'0000-00-00' limit 1";
  $rs=sqlQuery($sql,array($this->encounter_id));
  return $rs['dt'];
  }
